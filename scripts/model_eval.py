...
def main(args):
    ...
    fig.set_figheight(8)
    fig.set_figwidth(5)
    ax[0].bar(loss_bin_edges[:-1], loss_hist, width=np.diff(loss_bin_edges), edgecolor="black", align="edge",label=r'$\pi^{\text{DA-RNN}}$')
    if compare_w_mlp:
        ax[0].bar(loss_bin_edges_mlp[:-1], loss_hist_mlp, width=np.diff(loss_bin_edges_mlp), edgecolor="black", alpha=0.5,align="edge",color='red',label=r'$\pi^{\text{MLP}}$')
    ax[0].set_ylabel('#')
    ...
    if compare_w_mlp:
        ...
        axins.bar(loss_bin_edges_mlp[:-1], loss_hist_mlp, width=np.diff(loss_bin_edges_mlp), edgecolor="black", alpha=0.5, align="edge",color='red')
    ...
    disp = ConfusionMatrixDisplay.from_predictions(y_true, y_pred, normalize='true',cmap='YlGnBu',im_kw={'vmax': 1.})
    disp.plot(ax=ax[1],cmap='YlGnBu',im_kw={'vmax': 1.},text_kw={'fontsize':12,'fontweight':'bold'})
